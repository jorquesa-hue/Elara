...
export class Repositories {
  ...
  async loadWorld(): Promise<WorldData> {
    ...
    // Feature-table reads tolerate a relation that doesn't exist yet (the app can
    // deploy ahead of a migration being applied to the live DB): the table loads
    // EMPTY and everything else still rehydrates, instead of the whole cold-start
    // failing. `tenant` stays strict via one() — if THAT is missing, the server is
    // pointed at the wrong database and should degrade loudly, not boot empty.
    const missing: string[] = [];
    const maybe = async (text: string, values: unknown[] = [tid]) => {
      try {
        return await this.q.query({ text, values });
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        const code = (e as { code?: string }).code;
        const m = /relation "([^"]+)" does not exist/i.exec(msg);
        if (code === '42P01' || m) {
          missing.push(m?.[1] ?? text.slice(0, 60));
    ...
    const unitTypes = (await maybe('select id, tenant_id, code, name, bedrooms, bathrooms, max_guests, area_sqm, base_rent_cents, description from unit_type where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, code: String(r['code']), name: String(r['name']), bedrooms: n(r['bedrooms']), bathrooms: n(r['bathrooms']), maxGuests: n(r['max_guests']), areaSqm: n(r['area_sqm']), baseRentCents: n(r['base_rent_cents']), description: s(r['description']) }));
    const properties = (await maybe('select id, tenant_id, code, name, address, entity_id from property where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, code: String(r['code']), name: String(r['name']), address: s(r['address']), entityId: s(r['entity_id']) }));
    const units = (await maybe('select id, tenant_id, label, code, active, type_id, property_id from unit where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, label: String(r['label']), code: s(r['code']), active: r['active'] == null ? undefined : Boolean(r['active']), typeId: s(r['type_id']), propertyId: s(r['property_id']) }));
    const guests = (await maybe('select id, tenant_id, full_name, code, email from guest where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, fullName: String(r['full_name']), code: s(r['code']), email: s(r['email']) }));
    const ratePlans = (await maybe('select id, tenant_id, name, kind, base_cents, currency, deposit_cents from rate_plan where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, name: String(r['name']), kind: String(r['kind']), baseCents: Number(r['base_cents']), currency: String(r['currency']), depositCents: n(r['deposit_cents']) }));
    const users = (await maybe('select id, tenant_id, code, display_name, role_id, active from app_user where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, code: String(r['code']), displayName: String(r['display_name']), roleId: String(r['role_id']), active: Boolean(r['active']) }));
    const customRoles = (await maybe('select tenant_id, role_id, name, description, permissions from custom_role where tenant_id = $1')).map((r) => ({ tenantId: tid, roleId: String(r['role_id']), name: String(r['name']), description: s(r['description']), permissions: arr(r['permissions']) as string[] }));
    const legalEntities = (await maybe('select id, tenant_id, role, name, tax_id from legal_entity where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, role: String(r['role']), name: String(r['name']), taxId: s(r['tax_id']) }));
    const parties = (await maybe('select id, tenant_id, kind, display_name, legal_name, tax_id, email, phone, attributes from party where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, kind: String(r['kind']), displayName: String(r['display_name']), legalName: s(r['legal_name']), taxId: s(r['tax_id']), email: s(r['email']), phone: s(r['phone']), attributes: obj(r['attributes']) }));
    const spaces = (await maybe('select id, tenant_id, parent_id, type, code, label, leasable, capacity, attributes from space where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, parentId: s(r['parent_id']), type: String(r['type']), code: String(r['code']), label: String(r['label']), leasable: Boolean(r['leasable']), capacity: n(r['capacity']), attributes: obj(r['attributes']) }));
    const chargeTypes = (await maybe('select id, tenant_id, code, name, receiving_entity_id, gl_account, recurring from charge_type where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, code: String(r['code']), name: String(r['name']), receivingEntityId: String(r['receiving_entity_id']), glAccount: String(r['gl_account']), recurring: Boolean(r['recurring']) }));

    const agIds = (await maybe('select id from agreement where tenant_id = $1 order by id')).map((r) => String(r['id']));
    const agreements = [];
    for (const id of agIds) {
      const [meta] = await maybe('select id, tenant_id, guest_id, unit_id from agreement where id = $1 and tenant_id = $2', [id, tid]);
      const evRows = await maybe('select seq, agreement_id, type, at, payload from agreement_event where agreement_id = $1 order by seq', [id]);
      agreements.push({
    ...
    const invRows = await maybe('select id, agreement_id, tenant_id, issued_at, due_at, currency, total_cents, paid_cents, status, receiving_entity_id from invoice where tenant_id = $1');
    const invoices = [];
    for (const r of invRows) {
      const lines = (await maybe('select description, account, amount_cents, charge_type from invoice_line where invoice_id = $1', [String(r['id'])])).map((l) => ({ description: String(l['description']), account: String(l['account']), amountCents: Number(l['amount_cents']), chargeType: s(l['charge_type']) }));
    ...
    const apPayments = (await maybe('select ap.id, ap.bill_id, ap.amount_cents, ap.method, ap.paid_at, ap.status from ap_payment ap join bill b on b.id = ap.bill_id where b.tenant_id = $1')).map((r) => ({ id: String(r['id']), billId: String(r['bill_id']), amountCents: Number(r['amount_cents']), method: String(r['method']), paidAt: toIso(r['paid_at']), status: String(r['status']) }));

    const agreementParties = (await maybe('select ap.agreement_id, ap.party_id, ap.role, ap.share_pct, ap.from_date, ap.to_date from agreement_party ap join agreement a on a.id = ap.agreement_id where a.tenant_id = $1')).map((r) => ({ agreementId: String(r['agreement_id']), partyId: String(r['party_id']), role: String(r['role']), sharePct: n(r['share_pct']), from: s(r['from_date']), to: s(r['to_date']) }));

    const pricingRules = (await maybe('select id, tenant_id, name, base_cents, min_cents, max_cents, weekend_factor_bps, occupancy_tiers, lead_time_tiers, los_discounts, seasons from pricing_rule where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, name: String(r['name']), baseCents: Number(r['base_cents']), minCents: n(r['min_cents']), maxCents: n(r['max_cents']), weekendFactorBps: n(r['weekend_factor_bps']), occupancyTiers: arr(r['occupancy_tiers']), leadTimeTiers: arr(r['lead_time_tiers']), losDiscounts: arr(r['los_discounts']), seasons: arr(r['seasons']) }));
    const purchaseOrders = (await maybe('select id, tenant_id, vendor_id, entity_id, created_at, expected_at, currency, total_cents, status, billed_cents, approved_at, received_at, closed_at, cancelled_at, memo, lines from purchase_order where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, vendorId: String(r['vendor_id']), entityId: s(r['entity_id']), createdAt: toIso(r['created_at']), expectedAt: r['expected_at'] == null ? undefined : toIso(r['expected_at']), currency: String(r['currency']), totalCents: Number(r['total_cents']), status: String(r['status']), billedCents: Number(r['billed_cents']), approvedAt: r['approved_at'] == null ? undefined : toIso(r['approved_at']), receivedAt: r['received_at'] == null ? undefined : toIso(r['received_at']), closedAt: r['closed_at'] == null ? undefined : toIso(r['closed_at']), cancelledAt: r['cancelled_at'] == null ? undefined : toIso(r['cancelled_at']), memo: s(r['memo']), lines: arr(r['lines']) as Array<{ description: string; account: string; amountCents: number }> }));
    const budgets = (await maybe('select id, tenant_id, account, period_start, period_end, amount_cents, label from budget where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, account: String(r['account']), periodStart: toDate(r['period_start']), periodEnd: toDate(r['period_end']), amountCents: Number(r['amount_cents']), label: s(r['label']) }));
    const prospects = (await maybe('select id, tenant_id, name, party_id, preferences from roommate_prospect where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, name: String(r['name']), partyId: s(r['party_id']), preferences: obj(r['preferences']) }));
    const leads = (await maybe('select id, tenant_id, name, source, stage, est_value_cents, party_id, created_at, updated_at, stage_at, lost_reason from crm_lead where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, name: String(r['name']), source: s(r['source']), stage: String(r['stage']), estValueCents: Number(r['est_value_cents']), partyId: s(r['party_id']), createdAt: toIso(r['created_at']), updatedAt: toIso(r['updated_at']), stageAt: obj(r['stage_at']), lostReason: s(r['lost_reason']) }));
    const applications = (await maybe('select id, tenant_id, lead_id, unit_id, applicant_name, applicant_email, income_cents, status, submitted_at, screening, decided_at, decided_by, adverse_action_reason from application where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, leadId: s(r['lead_id']), unitId: s(r['unit_id']), applicantName: String(r['applicant_name']), applicantEmail: s(r['applicant_email']), incomeCents: n(r['income_cents']), status: String(r['status']), submittedAt: toIso(r['submitted_at']), screening: r['screening'] == null ? undefined : obj(r['screening']), decidedAt: r['decided_at'] == null ? undefined : toIso(r['decided_at']), decidedBy: s(r['decided_by']), adverseActionReason: s(r['adverse_action_reason']) }));
    const tours = (await maybe('select id, tenant_id, lead_id, unit_id, prospect_name, prospect_email, scheduled_at, status, agent_id, notes, created_at, completed_at, cancel_reason from tour where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, leadId: s(r['lead_id']), unitId: s(r['unit_id']), prospectName: String(r['prospect_name']), prospectEmail: s(r['prospect_email']), scheduledAt: toIso(r['scheduled_at']), status: String(r['status']), agentId: s(r['agent_id']), notes: s(r['notes']), createdAt: toIso(r['created_at']), completedAt: r['completed_at'] == null ? undefined : toIso(r['completed_at']), cancelReason: s(r['cancel_reason']) }));
    const unitTurns = (await maybe('select id, tenant_id, unit_id, status, vacated_at, ready_at, tasks, agent_id, notes, created_at, cancel_reason from unit_turn where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, unitId: String(r['unit_id']), status: String(r['status']), vacatedAt: toIso(r['vacated_at']), readyAt: r['ready_at'] == null ? undefined : toIso(r['ready_at']), tasks: arr(r['tasks']) as Array<{ key: string; label: string; done: boolean; doneAt?: string }>, agentId: s(r['agent_id']), notes: s(r['notes']), createdAt: toIso(r['created_at']), cancelReason: s(r['cancel_reason']) }));
    const pmSchedules = (await maybe('select id, tenant_id, title, space_id, cadence_days, priority, next_due_at, last_run_at, active, created_at from pm_schedule where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, title: String(r['title']), spaceId: s(r['space_id']), cadenceDays: Number(r['cadence_days']), priority: String(r['priority']), nextDueAt: toDate(r['next_due_at']), lastRunAt: r['last_run_at'] == null ? undefined : toIso(r['last_run_at']), active: r['active'] !== false, createdAt: toIso(r['created_at']) }));
    const insurancePolicies = (await maybe('select id, tenant_id, agreement_id, party_id, carrier, policy_number, liability_cents, effective_at, expires_at, status, verified_at, notes, created_at from insurance_policy where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, agreementId: String(r['agreement_id']), partyId: s(r['party_id']), carrier: String(r['carrier']), policyNumber: String(r['policy_number']), liabilityCents: Number(r['liability_cents']), effectiveAt: toDate(r['effective_at']), expiresAt: toDate(r['expires_at']), status: String(r['status']), verifiedAt: r['verified_at'] == null ? undefined : toIso(r['verified_at']), notes: s(r['notes']), createdAt: toIso(r['created_at']) }));
    const utilityBills = (await maybe('select id, tenant_id, property_id, utility, period_start, period_end, total_cents, method, status, billed_at, notes, created_at from utility_bill where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, propertyId: String(r['property_id']), utility: String(r['utility']), periodStart: toDate(r['period_start']), periodEnd: toDate(r['period_end']), totalCents: Number(r['total_cents']), method: String(r['method']), status: String(r['status']), billedAt: r['billed_at'] == null ? undefined : toIso(r['billed_at']), notes: s(r['notes']), createdAt: toIso(r['created_at']) }));
    const parcels = (await maybe('select id, tenant_id, party_id, agreement_id, carrier, tracking_number, description, location, status, received_at, notified_at, picked_up_at, picked_up_by, notes from parcel where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, partyId: String(r['party_id']), agreementId: s(r['agreement_id']), carrier: String(r['carrier']), trackingNumber: s(r['tracking_number']), description: s(r['description']), location: s(r['location']), status: String(r['status']), receivedAt: toIso(r['received_at']), notifiedAt: r['notified_at'] == null ? undefined : toIso(r['notified_at']), pickedUpAt: r['picked_up_at'] == null ? undefined : toIso(r['picked_up_at']), pickedUpBy: s(r['picked_up_by']), notes: s(r['notes']) }));
    const waitlist = (await maybe('select id, tenant_id, type_id, property_id, prospect_name, prospect_email, prospect_phone, desired_move_in, status, joined_at, offered_at, converted_at, lead_id, notes from waitlist_entry where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, typeId: s(r['type_id']), propertyId: s(r['property_id']), prospectName: String(r['prospect_name']), prospectEmail: s(r['prospect_email']), prospectPhone: s(r['prospect_phone']), desiredMoveIn: r['desired_move_in'] == null ? undefined : toDate(r['desired_move_in']), status: String(r['status']), joinedAt: toIso(r['joined_at']), offeredAt: r['offered_at'] == null ? undefined : toIso(r['offered_at']), convertedAt: r['converted_at'] == null ? undefined : toIso(r['converted_at']), leadId: s(r['lead_id']), notes: s(r['notes']) }));
    const distributions = (await maybe('select id, tenant_id, entity_id, property_id, amount_cents, currency, period_start, period_end, memo, recorded_at from distribution where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, entityId: String(r['entity_id']), propertyId: s(r['property_id']), amountCents: Number(r['amount_cents']), currency: String(r['currency']), periodStart: r['period_start'] == null ? undefined : toDate(r['period_start']), periodEnd: r['period_end'] == null ? undefined : toDate(r['period_end']), memo: s(r['memo']), recordedAt: toIso(r['recorded_at']) }));
    const contributions = (await maybe('select id, tenant_id, entity_id, property_id, amount_cents, currency, memo, recorded_at from contribution where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, entityId: String(r['entity_id']), propertyId: s(r['property_id']), amountCents: Number(r['amount_cents']), currency: String(r['currency']), memo: s(r['memo']), recordedAt: toIso(r['recorded_at']) }));
    const propertyBudgets = (await maybe('select id, tenant_id, property_id, period_start, period_end, currency, lines, notes, created_at from property_budget where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, propertyId: String(r['property_id']), periodStart: toDate(r['period_start']), periodEnd: toDate(r['period_end']), currency: String(r['currency']), lines: arr(r['lines']) as Array<{ category: 'revenue' | 'expense'; label: string; account?: string; amountCents: number }>, notes: s(r['notes']), createdAt: toIso(r['created_at']) }));
    const integrations = (await maybe('select id, tenant_id, kind, provider, status, config, secret_ref, created_at from integration where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, kind: String(r['kind']), provider: String(r['provider']), status: String(r['status']), config: obj(r['config']), secretRef: s(r['secret_ref']), createdAt: toIso(r['created_at']) }));
    const connectorCommands = (await maybe('select id, tenant_id, integration_id, action, payload, status, created_at, dispatched_at, resolved_at, result from connector_command where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, integrationId: String(r['integration_id']), action: String(r['action']), payload: obj(r['payload']), status: String(r['status']), createdAt: toIso(r['created_at']), dispatchedAt: r['dispatched_at'] == null ? undefined : toIso(r['dispatched_at']), resolvedAt: r['resolved_at'] == null ? undefined : toIso(r['resolved_at']), result: r['result'] == null ? undefined : obj(r['result']) }));
    const signatureEnvelopes = (await maybe('select id, tenant_id, document_name, provider, provider_ref, lead_id, agreement_id, signers, status, created_at, sent_at, completed_at, void_reason, decline_reason from signature_envelope where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, documentName: String(r['document_name']), provider: String(r['provider']), providerRef: s(r['provider_ref']), leadId: s(r['lead_id']), agreementId: s(r['agreement_id']), signers: arr(r['signers']), status: String(r['status']), createdAt: toIso(r['created_at']), sentAt: r['sent_at'] == null ? undefined : toIso(r['sent_at']), completedAt: r['completed_at'] == null ? undefined : toIso(r['completed_at']), voidReason: s(r['void_reason']), declineReason: s(r['decline_reason']) }));

    const workOrders = (await maybe('select id, tenant_id, space_id, title, description, category, priority, status, requested_by_party_id, assigned_vendor_party_id, bill_id, opened_at, assigned_at, started_at, closed_at, resolution, cancel_reason from work_order where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, spaceId: s(r['space_id']), title: String(r['title']), description: s(r['description']), category: s(r['category']), priority: String(r['priority']), status: String(r['status']), requestedByPartyId: s(r['requested_by_party_id']), assignedVendorPartyId: s(r['assigned_vendor_party_id']), billId: s(r['bill_id']), openedAt: toIso(r['opened_at']), assignedAt: r['assigned_at'] == null ? undefined : toIso(r['assigned_at']), startedAt: r['started_at'] == null ? undefined : toIso(r['started_at']), closedAt: r['closed_at'] == null ? undefined : toIso(r['closed_at']), resolution: s(r['resolution']), cancelReason: s(r['cancel_reason']) }));
    const notifications = (await maybe('select id, tenant_id, channel, recipient, kind, data, status, created_at, sent_at, failed_reason, provider_ref from notification where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, channel: String(r['channel']), to: String(r['recipient']), kind: String(r['kind']), data: obj(r['data']), status: String(r['status']), createdAt: toIso(r['created_at']), sentAt: r['sent_at'] == null ? undefined : toIso(r['sent_at']), failedReason: s(r['failed_reason']), providerRef: s(r['provider_ref']) }));
    ...
    const actionLog = (await maybe('select seq, tenant_id, at, actor, action, effect, rule_id, outcome, reason, exception_id from action_log where tenant_id = $1 order by seq')).map((r) => ({ seq: Number(r['seq']), tenantId: tid, at: toIso(r['at']), actor: String(r['actor']), action: String(r['action']), effect: String(r['effect']), ruleId: s(r['rule_id']) ?? null, outcome: String(r['outcome']), reason: String(r['reason']), exceptionId: s(r['exception_id']) ?? null }));

    const periodLocks = (await maybe('select tenant_id, period, status, closed_at, closed_by, reopened_at, reopened_by from period_lock where tenant_id = $1')).map((r) => ({ tenantId: tid, period: String(r['period']), status: String(r['status']), closedAt: s(r['closed_at']) ? toIso(r['closed_at']) : undefined, closedBy: s(r['closed_by']), reopenedAt: s(r['reopened_at']) ? toIso(r['reopened_at']) : undefined, reopenedBy: s(r['reopened_by']) }));
    const bankAccounts = (await maybe('select id, tenant_id, code, name, kind, gl_account, entity_id from bank_account where tenant_id = $1')).map((r) => ({ id: String(r['id']), tenantId: tid, code: String(r['code']), name: String(r['name']), kind: String(r['kind']), glAccount: String(r['gl_account']), entityId: s(r['entity_id']) }));
    if (missing.length) console.warn(`loadWorld(${tid}): ${missing.length} relation(s) not in the live schema — loaded empty; apply the latest migrations: ${missing.join(', ')}`);
    return { tenants, units, unitTypes, properties, guests, ratePlans, users, customRoles, legalEntities, parties, spaces, chargeTypes, agreements, holds, journalLines, invoices, payments, deposits, bills, apPayments, agreementParties, pricingRules, purchaseOrders, budgets, prospects, leads, applications, tours, unitTurns, pmSchedules, insurancePolicies, utilityBills, parcels, waitlist, distributions, contributions, propertyBudgets, integrations, connectorCommands, signatureEnvelopes, workOrders, notifications, exceptions, periodLocks, bankAccounts, actionLog } as unknown as WorldData;
  }
