...
export const POLICY_RULES: readonly PolicyRule[] = [
  ...
  {
    id: 'pol-deposit-refund-large',
    action: 'deposit.refund',
    effect: 'escalate',
    description: 'Refunding more than R$5,000 of a security deposit sends real money back to the resident: human approval required (parallels bill.pay).',
    when: (ctx) => (ctx.amountCents ?? 0) > 500_000,
    conditionNote: 'amount_cents > 500000',
  },
  { id: 'pol-deposit-refund', action: 'deposit.refund', effect: 'allow', description: 'Deposit refunds with itemized deductions are routine under the threshold.' },
  ...
  {
    id: 'pol-connector-dispatch-payout',
    action: 'connector.dispatch',
    effect: 'escalate',
    description: 'A money-moving command to a bank or payment-gateway integration above R$5,000 is a real payout/charge over an external rail — the same money-out control as bill.pay applies: human approval required. Closes the parallel rail that would otherwise bypass the AP escalation.',
    when: (ctx) =>
      (ctx.integrationKind === 'bank' || ctx.integrationKind === 'payment_gateway') &&
      (ctx.amountCents ?? 0) > 500_000,
    conditionNote: 'integration_kind in (bank,payment_gateway) and amount_cents > 500000',
  },
